// --------------------------------------------------------------------------
// Initializes name functions
// tied to Flask app
//1 - populates selector
// --------------------------------------------------------------------------
//populate the options of a select element in html  using javascript
//You'll first get the dropdown element from the DOM, then loop through the array, 
//and add each element as a new option in the dropdown like this:
// --------------------------------------------------------------------------
...
function updatePie(newValues, newLabels, newNames, sample_name){
    ...
    Plotly.restyle("pie", "hovertext", [newNames]);
    Plotly.relayout("pie", "title", "Top Sample Counts for " + sample_name);
...
function updateBub(values, labels, names, sample_name){
    ...
    Plotly.restyle("bubble", "text", [names]);
    Plotly.relayout("bubble", "title", "Sample Values for " + sample_name);
...
function updateMeter(newWashFreq) {
    ...
    console.log("Wash Frequency Gauge Updated!");};
...
function getPointerCoords(targetSample) {
  ...
  // calculates gauge  pointer location using trigonometric functions
